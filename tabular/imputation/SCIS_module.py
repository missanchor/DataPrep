...
def _train_step_gain(generator, discriminator, x_mb, m_mb, opt_g, opt_d, params, device):
    ...
    g_loss = -torch.mean((1 - m_mb) * torch.log(d_prob + 1e-8)) + \
             params['alpha'] * torch.mean((m_mb * x_mb - m_mb * g_sample) ** 2) / (torch.mean(m_mb) + 1e-8) + \
             params['value'] * sinkhorn_loss_torch(x_mb, g_sample, epsilon=params['epsilon'])
    g_loss.backward()
    opt_g.step()
...
def train_scis_algorithm(generator, discriminator, data_x, mask, params, device):
    ...
    for _ in tqdm(range(params['epoch']), desc="Initial"):
        ...
        for i in range(0, initial_num, params['batch_size']):
            ...
            _train_step_gain(generator, discriminator, torch.tensor(data_x[mb_idx], dtype=torch.float32).to(device),
                             torch.tensor(mask[mb_idx], dtype=torch.float32).to(device), opt_g, opt_d, params, device)
    ...
    print(f"Phase 3: Retraining ({final_n} samples)...")
    train_idx = np.random.permutation(no)[:final_n]
    for _ in tqdm(range(params['epoch']), desc="Retraining"):
